pca_cr <- function(CR, threshold = 0.99, center = TRUE, scale. = TRUE) {
  stopifnot(is.matrix(CR) || is.data.frame(CR))
  CR <- as.matrix(CR)
  
  fit <- prcomp(CR, center = center, scale. = scale.)
  
  var_expl <- fit$sdev^2
  prop_expl <- var_expl / sum(var_expl)
  cum_prop  <- cumsum(prop_expl)
  
  k <- which(cum_prop >= threshold)[1]
  
  list(
    k = k,
    threshold = threshold,
    prop_explicada = prop_expl,
    prop_cumulativa = cum_prop,
    loadings = fit$rotation[, 1:k, drop = FALSE],
    scores = fit$x[, 1:k, drop = FALSE],
    sdev = fit$sdev,
    prcomp = fit
  )
}
...
IKPCA_CR_PCA <- function(separation_method = c("SVM", "LDA"),
                     data_train,
                     data_test,
                     label_train,
                     label_test,
